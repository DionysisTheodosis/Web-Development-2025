...
//    }
    ...
    @ResponseStatus(HttpStatus.UNAUTHORIZED)
    public @ResponseBody ErrorResponse BadCredentialsException(
    ...
    public ResponseEntity<String> handleJsonErrors(HttpMessageNotReadableException ex) {
        // Switch directly on the CAUSE of the error
        return switch (ex.getCause()) {

            case InvalidFormatException ifx
                    when ifx.getTargetType() == CarType.class ->
                    ResponseEntity.badRequest().body("Invalid Car Type. Allowed: PASSENGER, TRUCK...");

            case InvalidFormatException ifx
                    when ifx.getTargetType() == FuelType.class ->
                    ResponseEntity.badRequest().body("Invalid Movement Type. Allowed: PETROL, ELECTRIC , ...");
            case InvalidFormatException ifx
                    when ifx.getTargetType() == UserRole.class ->
                    ResponseEntity.badRequest().body("Invalid User Role. Allowed: SECRETARY, PATIENT, DOCTOR...");

            case null, default ->
                    ResponseEntity.badRequest().body("Invalid JSON format or unknown error.");
        };
    }
